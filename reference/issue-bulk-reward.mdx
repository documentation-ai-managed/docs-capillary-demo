...
| `customFields` | Object | The list of custom fields associated with the reward issued to the customer. A custom field allows you to add extra information related to the reward. You can add the custom field in the issue reward call once the custom field is created using the [create custom field API](/reference/post-create-custom-field). |
...
<CodeGroup show-lines="true" tabs={["json", "With payment config", "With fullfilment status and custom field", "With user_group2_primary_user_id", "With params in Reward Level", "With params in Request Level", "With redemptionValue", "With event log ID and request ID", "Issuing a reward by defining  Quantity and Redemption Value", "Issuing a reward with only quantity"]}>
...
<CodeGroup show-lines="true" tabs={["200 OK", "Reward issued partially", "Invalid payment mode", "Payment config ID mandatory", "User don't belong to group", "Non-Redeemable points", "Invalid ID is passed", "Invalid Request ID", "Duplicate Request ID", "User not associated with the card series linked to reward"]}>
