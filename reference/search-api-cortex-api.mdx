...
This API is used when you want only matching customer or transaction ID values in the results for a given search. Further, if you want to add additional information to the search result, use the [Datafield API](/reference/create-data-field) to enrich the search result.
