---
title: Get Transaction Details by Reward Transaction ID in Connected orgs
---

This API is used to retrieve details of a specific reward transaction using reward transaction ID, in connected orgs.

This API retrieves the detailed information of a specific reward transaction using the reward transaction ID.

<Callout kind="tip" title="Note">

For detailed information about our APIs and for hands-on testing, refer documentation in [API overview](https://docs.capillarytech.com/reference/apioverview) and  step-by-step guide on making your first API call in [Make your first API call](https://docs.capillarytech.com/reference/make-your-first-api-call) .

</Callout>

<Callout kind="alert" title="**Warning**">

**Warning**

Use this API exclusively for connected orgs. For other orgs, use this [API](/reference/get-reward-issue-transaction-details) instead.

</Callout>

## Prerequisites

* [ ] Authentication: Basic or OAuth authentication
* [ ] Default access group

## Resource information

|                        |                                                                              |
| :--------------------- | :--------------------------------------------------------------------------- |
| URL                    | /api_gateway/rewards/core/v1.1/reward-transactions`{rewardTransactionId}` |
| HTTP Method            | GET                                                                          |
| Pagination             | Yes                                                                          |
| Batch support          | No                                                                           |
| Rate limit information | None                                                                         |

## API endpoint example

&lt;[https://eu.api.capillarytech.com/api_gateway/rewards/core/v1.1/reward-transactions/32787](https://eu.api.capillarytech.com/api_gateway/rewards/core/v1.1/reward-transactions/32787)>

## Headers

| Header | Description |
| --- | --- |
| DATA-SCOPE-ORG | List of Organization IDs |
| DATA-SCOPE | Scopes define what data can be accessed using the API. You can use scopes to control access to data from a parent or child organization. Defining a scope ensures that the response contains only data from the respective organization. Supported headers: `SELF`, `OTHER` and `ALL`. Refer to connected orgs [data scopes](https://docs.capillarytech.com/reference/connected-orgs-data-scopes) for more information. |

## Request path parameters

| Parameters            | Data Type | Description                                                                                                                           |
| :-------------------- | :-------- | :------------------------------------------------------------------------------------------------------------------------------------ |
| rewardTransactionId* | String    | Unique identifier of the reward transaction. The reward transaction is generated during the issual of the reward. **Example**: 327847 |

<br />

<CodeGroup show-lines="true" tabs={["curl", "With couponCodeCase filter"]}>

  ```bash
  /curl --location 'http://crm-nightly-new.cc.capillarytech.com/api_gateway/rewards/core/v1.1/reward-transactions/<rewardTransactionId>' \
  e--header 'DATA-SCOPE: OTHER' \
  x--header 'DATA-SCOPE-ORG: 50672' \
  --header 'Authorization: Basic a2FyXzE6MzE1MmQyMzRiNzA='
  ```

  ```text
  curl --location 'https://crm-staging-new.cc.capillarytech.com/api_gateway/rewards/core/v1.1/reward-transactions/927488?couponCodeCase=AS_IT_IS' \
  --header 'Accept: application/json' \
  --header 'Content-Type: application/json' \
  --header 'DATA-SCOPE: ALL' \
  --header 'Authorization: Basic dGlsbGF1dG9fMTIzOjI3NDFlNjJiZTZiZTYyNGY2N2IwMGQxZDZhNWQ2MWJm'
  ```

</CodeGroup>

## Request query parameter

| Parameter | Type | Description |
| --- | --- | --- |
| `couponCodeCase` | Enum | This parameter helps retrieve the coupon code in the desired letter cases. By default, the system automatically converts the coupon code to uppercase. Supported Values: `LOWER`, `UPPER`, `AS_IT_IS` `LOWER` - Converts the coupon code to lowercase. Example: `"ABCD1234"` → `"abcd1234"` `UPPER` - Converts the coupon code to uppercase. Example: `"abcd1234"` → `"ABCD1234" `AS_IT_IS` - Returns the coupon code exactly as it was created. Example: `"abAB123"` → `"abAB123"` |

## Response parameters

| Parameter Name                | Description                                                                                                                                                                                                                                                                                                                             |
| :---------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`status`**                  | Contains the details of the operation.                                                                                                                                                                                                                                                                                                  |
| `success`                     | Indicates if the operation was successful.                                                                                                                                                                                                                                                                                              |
| `code`                        | HTTP status code of the operation.                                                                                                                                                                                                                                                                                                      |
| `message`                     | Message about the operation's outcome.                                                                                                                                                                                                                                                                                                  |
| **`data`**                    | Contains detailed information about the reward transaction.                                                                                                                                                                                                                                                                             |
| `rewardTransactionId`         | Unique identifier for the reward transaction. It is generated during the issuance of the reward.                                                                                                                                                                                                                                        |
| `-customerId`                 | Unique identifier for the customer.                                                                                                                                                                                                                                                                                                     |
| `-orgId`                      | Unique identifier for the organization.                                                                                                                                                                                                                                                                                                 |
| `-rewardIssueRefId`           | Reference ID for the reward issuance.                                                                                                                                                                                                                                                                                                   |
| `-rewardTransactionDate`      | Date and time of the reward transaction in milliseconds since epoch (Unix timestamp format).                                                                                                                                                                                                                                            |
| `-status`                     | Contains the status of the reward issuance.                                                                                                                                                                                                                                                                                             |
| `--success`                   | Indicates if the reward was issued successfully: `true` for successful, `false` for failed.                                                                                                                                                                                                                                             |
| `--code`                      | Status code for the reward issue transaction.                                                                                                                                                                                                                                                                                           |
| `--message`                   | Status message for the reward issue transaction, indicating if the reward issuance was successful.                                                                                                                                                                                                                                      |
| `-quantity`                   | Number of rewards issued.                                                                                                                                                                                                                                                                                                               |
| `-requestedQuantity`          | Represents the original reward quantity that was requested.                                                                                                                                                                                                                                                                             |
| `-groupRedemption`            | Indicates if the redemption is a group redemption.                                                                                                                                                                                                                                                                                      |
| `-rewardDetails`              | Contains details of the issued reward.                                                                                                                                                                                                                                                                                                  |
| `--rewardId`                  | Unique identifier of the reward.                                                                                                                                                                                                                                                                                                        |
| `--languageCode`              | Language code of the reward details. **Example**: `en` for English.                                                                                                                                                                                                                                                                     |
| `--name`                      | Name of the reward.                                                                                                                                                                                                                                                                                                                     |
| `--description`               | Description of the reward.                                                                                                                                                                                                                                                                                                              |
| `--rewardRank`                | Rank or position of the reward within its category or list.                                                                                                                                                                                                                                                                             |
| `--groups`                    | Specifies the group associated with the reward.                                                                                                                                                                                                                                                                                         |
| `--imageId`                   | Unique identifier for the image associated with the reward.                                                                                                                                                                                                                                                                             |
| `--imageUrl`                  | URL to the image associated with the reward.                                                                                                                                                                                                                                                                                            |
| `--intouchPoints`             | The number of intouch points associated with the reward.                                                                                                                                                                                                                                                                                |
| `--redemptionType`            | Gives the redemption type of the reward. **Example**: `MILES`.                                                                                                                                                                                                                                                                          |
| `--images`                    | A list of image details related to the reward.                                                                                                                                                                                                                                                                                          |
| `--name`                      | Name of the image.                                                                                                                                                                                                                                                                                                                      |
| `--altText`                   | Alternative text or description of the image.                                                                                                                                                                                                                                                                                           |
| `--id`                        | Identifier for internally stored images, generated when uploaded to the file service. Not applicable for external images.                                                                                                                                                                                                               |
| `--url`                       | URL of the image.                                                                                                                                                                                                                                                                                                                       |
| `--isExternal`                | Indicates if the image is hosted on an external server.                                                                                                                                                                                                                                                                                 |
| `--videos`                    | A list of videos associated with the reward.                                                                                                                                                                                                                                                                                            |
| `--name`                      | Name of the video.                                                                                                                                                                                                                                                                                                                      |
| `--altText`                   | Alternative text or description of the video.                                                                                                                                                                                                                                                                                           |
| `--id`                        | Identifier for internally stored images, generated when uploaded to the file service. Not applicable for external videos.                                                                                                                                                                                                               |
| `--url`                       | URL of the video.                                                                                                                                                                                                                                                                                                                       |
| `--isExternal`                | Indicates if the video is hosted on an external server.                                                                                                                                                                                                                                                                                 |
| `-pointsRedemption`           | Contains details of the points redemption process.                                                                                                                                                                                                                                                                                      |
| `--redeemedPoints`            | Number of points redeemed.                                                                                                                                                                                                                                                                                                              |
| `--redeemedPointsInDecimal`   | Number of points redeemed, expressed as a decimal value rounded to 3 decimal places. Rounding follows the HALF_UP method, where if the fourth decimal digit is 5 or more, the third decimal rounds up. For example, 21.4555 becomes 21.456, and 21.4554 becomes 21.455. This field will have values only for conversion ratio rewards. |
| `--pointsRedemptionRefId`     | Unique identifier generated each time rewards are issued. This field stores a list of these identifiers, allowing you to track multiple points redemption transactions.                                                                                                                                                                 |
| `--status`                    | Status details of the points redemption.                                                                                                                                                                                                                                                                                                |
| `---success`                  | Indicates whether the points redemption was successful.                                                                                                                                                                                                                                                                                 |
| `---code`                     | A unique code representing the status of the points redemption.                                                                                                                                                                                                                                                                         |
| `---message`                  | A message providing information about the points redemption status. **Example**: `Points redeemed successfully.`                                                                                                                                                                                                                        |
| `-redemptionDetails`          | Contains details of the redemption process.                                                                                                                                                                                                                                                                                             |
| `--redemptionType`            | Gives the redemption type of the reward. **Example**: `INTOUCH_REWARD`.                                                                                                                                                                                                                                                                 |
| `--totalRedemptionValue`      | The total value of the redemption. **Example**: `10` miles.                                                                                                                                                                                                                                                                             |
| `-vouchers`                   | A list of vouchers associated with the redemption.                                                                                                                                                                                                                                                                                      |
| `vouchers.code`               | The unique code for the voucher. The casing of the coupon code is based on the `couponCodeCase` query parameter.                                                                                                                                                                                                                        |
| `vouchers.codeExpiry`         | The expiration date of the voucher. Format: "YYYY-MM-DD HH:MM:SS".                                                                                                                                                                                                                                                                      |
| `vouchers.codeExpiryDateTime` | The expiration date of the voucher in ISO 8601 format.                                                                                                                                                                                                                                                                                  |
| `-vendorRewards`              | Stores the vendor reward details.                                                                                                                                                                                                                                                                                                       |
| `--rewards`                   | Identifier for the vendor reward.                                                                                                                                                                                                                                                                                                       |
| `-customFields`               | Custom fields for additional information.                                                                                                                                                                                                                                                                                               |
| `-transactionCustomFields`    | Custom fields associated with the transaction.                                                                                                                                                                                                                                                                                          |
| `-ownerId`                    | Unique identifier of the owner. **Example**: `123`.                                                                                                                                                                                                                                                                                     |

<br />

<CodeGroup show-lines="true" tabs={["Sample response", "With couponCodeCase filter"]}>

  ```json
  {
      "status": [
          {
              "success": true,
              "code": 200,
              "message": "Reward issue transactions details fetched successfully"
          }
      ],
      "data": {
          "rewardTransactionId": "327847",
          "customerId": 555738346,
          "orgId": 100458,
          "rewardIssueRefId": "transactionNumber",
          "rewardTransactionDate": 1737629567000,
          "status": {
              "success": true,
              "code": 200,
              "message": "Reward issued successfully"
          },
          "quantity": 1,
          "requestedQuantity": 1,
          "groupRedemption": false,
          "rewardDetails": {
              "rewardId": 295211,
              "languageCode": "en",
              "name": "Donate to Charity : Help us brighten a child's day!",
              "description": "Redeem you points and donate to Charity, 1 Point = 1 AED",
              "rewardRank": 1,
              "groups": [],
              "imageId": "5d05ccb2-6f5b-4b58-ad90-b97647c86784",
              "imageUrl": "https://eucrm-solutions.s3.amazonaws.com/rewards/1aba92d2-28bc-44dc-9dc9-00fbf662.jpg",
              "intouchPoints": 0,
              "redemptionType": "MILES",
              "images": [],
              "videos": []
          },
          "pointsRedemption": {
              "redeemedPoints": 30,
              "redeemedPointsInDecimal": 30.000,
              "pointsRedemptionRefId": "kLZgHE",
              "pointsRedemptionRefIds": [
                  "kLZgHE"
              ],
              "status": {
                  "success": true,
                  "code": 200,
                  "message": "Points redeemed successfully."
              }
          },
          "redemptionDetails": {
              "redemptionType": "MILES",
              "totalRedemptionValue": 10
          },
          "vouchers": [],
          "vendorRewards": {
              "rewards": [
                  {
                      "json": "{}"
                  }
              ]
          },
          "customFields": {
              "A": "1"
          },
          "transactionCustomFields": {
              "testdate": "2020-01-01 01:01:01"
          },
          "ownerId": ""
      }
  }
  ```

  ```text
  {
      "status": [
          {
              "success": true,
              "code": 200,
              "message": "Reward issue transactions details fetched successfully"
          }
      ],
      "data": {
          "rewardTransactionId": "927488",
          "customerId": 345635195,
          "orgId": 50339,
          "rewardIssueRefId": "1718950196",
          "rewardTransactionDate": 1747372097000,
          "status": {
              "success": true,
              "code": 200,
              "message": "Reward issued successfully"
          },
          "quantity": 1,
          "requestedQuantity": 1,
          "groupRedemption": false,
          "rewardDetails": {
              "rewardId": 139831,
              "languageCode": "en",
              "name": "english rewardName",
              "description": "hello",
              "groups": [],
              "imageId": "EXDR12987U",
              "imageUrl": "https://s3.amazonaws.com/crm-staging-new-solutions/AutomationBrandMarvel/EXDR12987U?X-Amz-Security-Token=IQoJb3JpZ2luX2VjEJH%2F%2F%2F%2F%2F%2F%2F%2F%2F%2FwEaCXVzLWVhc3QtMSJHMEUCIE%2BeRWPDz0XYx7IjQEri%2F%2FpzAhgGJvwXWBc7mSOrRaokAiEAu1INaG1iTdyTeXZpI%2BZ%2BsNEI7VdoVdG1J53VmL4XhGAqvwIIWhAFGgw0ODY0NTY5ODYyNjYiDDcco1G%2BCL1zwPK2YCqcAvsLh13E3dAKtZs5Ru9O3VO8xXheuSsjubDb6csrxpnOT2cQsVnN3ma8Q1Pw7oG83pY717SdEVaRP951tkv9XwGgiB3sNCL1cs1AwLf2BDRvLvrb24PKzcJG3iQszRjFMHpujBuAiJt53AzY9Qy%2F6WlOXCRuuMVlUMeGdQNforRTTAkujk3kUtpsNzc%2B6gzDl04Gv2mlaznSAXweADW0cS6ajn79loWUlnaITZIwjJQUVxm4xgxFtki0%2BEhS%2FS1LnxHH3riQ%2Fixza0%2BvoHoQME8EbVXDEADWuS3ZeSVw50rQtult00p%2BIwjZt7IyM4RC0Xmh5lwKeZOcaKyhu762jyGRBgH8veMlXKTNNdfxvzqfV1ddB0x1hgbAIIYDMIz61cEGOp0BSEegrOVeFJSjbBOTD%2Fy2W1l0cN6PCvrap9NRmv3ws8pdbS89nSgPYdzF8hv6t%2F4fALargH1mUSahDGLUhSgmMH%2FcCIoXSb2iL4kJRycgr9j98hRQiJvdxh%2BHHIMimrvmDqpPzxOyZ0Ee7WDrSW9h1qsP5YD12cWAuU4hFvuM%2By077e1crCfa7EeNQHg2rKY04IvkxahZmvpcxVg9Dw%3D%3D&X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Date=20250527T085416Z&X-Amz-SignedHeaders=host&X-Amz-Expires=3600&X-Amz-Credential=ASIAXCQYYRKNHNOLXL4J%2F20250527%2Fus-east-1%2Fs3%2Faws4_request&X-Amz-Signature=c2a7ac120eddfac2cebd3ddd16dc2c8453af7598a923ee8b7591db92b931e107",
              "intouchPoints": 0,
              "intouchSeriesId": "96303",
              "redemptionType": "INTOUCH_REWARD",
              "images": [],
              "videos": []
          },
          "pointsRedemption": {
              "redeemedPoints": 0,
              "vendorName": "test004",
              "pointsRedemptionRefIds": [
                  null
              ],
              "status": {
                  "success": true,
                  "code": 200,
                  "message": "Points redeemed successfully."
              },
              "vendorId": 37
          },
          "redemptionDetails": {
              "redemptionType": "INTOUCH_REWARD",
              "totalRedemptionValue": null
          },
          "vouchers": [
              {
                  "code": "UPPERlower003",
                  "codeExpiry": "2025-06-15 00:00:00",
                  "codeExpiryDateTime": "2025-06-15T00:00:00Z",
                  "redemptionDetails": {
                      "redemptionType": "INTOUCH_REWARD",
                      "redemptionValue": null
                  }
              }
          ],
          "vendorRewards": {
              "rewards": []
          },
          "customFields": {
              "CF2_eebced882025-05-27 13:57:18.344501": "defaultValue_2",
              "CF1_50a6f4cf2025-05-27 13:57:18.344487": "defaultValue_1"
          },
          "richContentRO": {},
          "ownerId": "",
          "eventDateTime": "2025-05-16T05:08:17Z",
          "issueDateTime": "2025-05-16T05:08:17Z"
      }
  }
  ```

</CodeGroup>

<br />

`<style>`
  .InputGroupe2EvBGz2cnqy \{
   display: flex;
   gap: 2px;
   position: relative;
\}

.Button.APIRequest-example-button1DGMsfaOTVNW \{
   position: relative;
   width: 140px;
   white-space: nowrap;
   overflow: hidden;
   text-overflow: ellipsis;
   padding: 6px 12px;
   text-align: left;
   transition: width 0.2s ease-out;
   z-index: 1;
   display: block;
   direction: ltr;
   unicode-bidi: embed;
   padding-right: 20px;
   background: #1e2329;
   border: none;
   border-radius: 6px;
   color: #94a3b8;
   font-size: 13px;
\}

.Button.APIRequest-example-button1DGMsfaOTVNW span,
.Button.APIRequest-example-button1DGMsfaOTVNW div \{
   text-align: left;
   direction: ltr;
\}

.Button.APIRequest-example-button1DGMsfaOTVNW:hover \{
   width: auto;
   z-index: 10;
   background: #262b33;
\}

.Button.APIRequest-example-button1DGMsfaOTVNW.APIRequest-example-button_activeXJeukHyaHZXB \{
   z-index: 20;
   background: #2c333d;
   color: #ffffff;
\}
`</style>`

## OpenAPI definition

```json
{
  "openapi": "3.1.0",
  "info": {
    "title": "v1",
    "version": "1.0"
  },
  "servers": [
    {
      "url": "https://{host}",
      "variables": {
        "host": {
          "default": "host"
        }
      }
    }
  ],
  "components": {
    "securitySchemes": {
      "sec0": {
        "type": "http",
        "scheme": "basic"
      }
    }
  },
  "security": [
    {
      "sec0": []
    }
  ],
  "paths": {
    "/api_gateway/rewards/core/v1.1/reward-transactions/{rewardTransactionId}": {
      "get": {
        "summary": "Get Transaction Details by Reward Transaction ID in Connected orgs",
        "description": "This API is used to retrieve details of a specific reward transaction using reward transaction ID, in connected orgs.",
        "operationId": "get-transaction-details-by-reward-transaction-id-in-connected-orgs",
        "parameters": [
          {
            "name": "DATA-SCOPE-ORG",
            "in": "header",
            "description": "List of Organization IDs",
            "schema": {
              "type": "string"
            }
          },
          {
            "name": "DATA-SCOPE",
            "in": "header",
            "description": "Scopes define what data can be accessed using the API. You can use scopes to control access to data from a parent or child organization. Defining a scope ensures that the response contains only data from the respective organization.  Supported headers: `SELF`, `OTHER` and `ALL`.",
            "schema": {
              "type": "string"
            }
          },
          {
            "name": "rewardTransactionId",
            "in": "path",
            "description": "Unique identifier of the reward transaction. The reward transaction is generated during the issual of the reward.",
            "schema": {
              "type": "string"
            },
            "required": true
          }
        ],
        "responses": {
          "200": {
            "description": "200",
            "content": {
              "application/json": {
                "examples": {
                  "Sample response with DATA-SCOPE as OTHER": {
                    "value": "{\n    \"status\": [\n        {\n            \"success\": true,\n            \"code\": 200,\n            \"message\": \"Reward issue transactions details fetched successfully\"\n        }\n    ],\n    \"data\": {\n        \"rewardTransactionId\": \"32695\",\n        \"orgId\" : 50672,\n        \"customerId\" : 123456,\n        \"rewardIssueRefId\": \"transactionNumber\",\n        \"rewardTransactionDate\": 1653906525000,\n        \"status\": {\n            \"success\": true,\n            \"code\": 200,\n            \"message\": \"Reward issued successfully\"\n        },\n        \"quantity\": 1,\n        \"requestedQuantity\": 0,\n        \"groupRedemption\": false,\n        \"rewardDetails\": {\n            \"rewardId\": 663,\n            \"languageCode\": \"en\",\n            \"name\": \" INTOUCH Enabled\",\n            \"description\": \"Description\",\n            \"groups\": [],\n            \"imageId\": \"EXDR12987U\",\n            \"imageUrl\": \"https://s3.amazonaws.com/crm-nightly-new-solutions/SAHANA_TEST/EXDR12987U?X-Amz-Security-Token=FwoGZXIvYXdzECEaDFH7mzvBmQUc2KqQryLWARZFNas29Qa%2BGT3bozjU69vumkm4K%2FACklgdn2iD2wQJKg3XS5P8SUKcwOriTGT6KXt2f8sq4VQGey7NBKPIfiwEC8YCkOdyt5iHPMt3Cdm1Op4ReXFKvJ2lPyOWEYS1lXYVYHhOHmKwFn%2BlTEIUgQBLna22wNZj4Pug8%2Fs8vBLw3Yf6vYvcrmoQpq0qpM9rPie3i4vMrDZgc03vB5mvlMNqXSwHtpW6X%2BNVX3%2FepQqCaPNj2gsy3%2BTZYK6FBnLyai7%2Fi5ndBqpp9B4Rpkc7%2Fj0T3D5xChAor%2BWMuQYyLRoAD76np90BetlC8Hx42juhaOJ9R1aiZ778N6i7cJhXw35VM6Cezs5uuf4UMQ%3D%3D&X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Date=20241031T073303Z&X-Amz-SignedHeaders=host&X-Amz-Expires=3599&X-Amz-Credential=ASIAXCQYYRKNOA4DTKYN%2F20241031%2Fus-east-1%2Fs3%2Faws4_request&X-Amz-Signature=5cf28d18d4b666d53f67455e5f54b1af794659c9be90d3e2492701c71a4a8d98\",\n            \"intouchPoints\": 0,\n            \"intouchSeriesId\": \"741888\",\n            \"redemptionType\": \"INTOUCH_REWARD\",\n            \"images\": [],\n            \"videos\": []\n        },\n        \"pointsRedemption\": {\n            \"redeemedPoints\": 0,\n            \"pointsRedemptionRefIds\": [\n                null\n            ],\n            \"status\": {\n                \"success\": true,\n                \"code\": 200,\n                \"message\": \"Points redeemed successfully.\"\n            }\n        },\n        \"vouchers\": [],\n        \"vendorRewards\": {\n            \"rewards\": []\n        },\n        \"ownerId\": \"\"\n    }\n}"
                  },
                  "With 'altText' for images and videos": {
                    "value": "{\n    \"status\": [\n        {\n            \"success\": true,\n            \"code\": 200,\n            \"message\": \"Reward issue transactions details fetched successfully\"\n        }\n    ],\n    \"data\": {\n        \"rewardTransactionId\": \"32695\",\n        \"orgId\" : 50672,\n        \"customerId\" : 123456,\n        \"rewardIssueRefId\": \"transactionNumber\",\n        \"rewardTransactionDate\": 1653906525000,\n        \"status\": {\n            \"success\": true,\n            \"code\": 200,\n            \"message\": \"Reward issued successfully\"\n        },\n        \"quantity\": 1,\n        \"requestedQuantity\": 0,\n        \"groupRedemption\": false,\n        \"rewardDetails\": {\n            \"rewardId\": 663,\n            \"languageCode\": \"en\",\n            \"name\": \" INTOUCH Enabled\",\n            \"description\": \"Description\",\n            \"groups\": [],\n            \"imageId\": \"EXDR12987U\",\n            \"imageUrl\": \"https://s3.amazonaws.com/crm-nightly-new-solutions/SAHANA_TEST/EXDR12987U?X-Amz-Security-Token=FwoGZXIvYXdzECEaDFH7mzvBmQUc2KqQryLWARZFNas29Qa%2BGT3bozjU69vumkm4K%2FACklgdn2iD2wQJKg3XS5P8SUKcwOriTGT6KXt2f8sq4VQGey7NBKPIfiwEC8YCkOdyt5iHPMt3Cdm1Op4ReXFKvJ2lPyOWEYS1lXYVYHhOHmKwFn%2BlTEIUgQBLna22wNZj4Pug8%2Fs8vBLw3Yf6vYvcrmoQpq0qpM9rPie3i4vMrDZgc03vB5mvlMNqXSwHtpW6X%2BNVX3%2FepQqCaPNj2gsy3%2BTZYK6FBnLyai7%2Fi5ndBqpp9B4Rpkc7%2Fj0T3D5xChAor%2BWMuQYyLRoAD76np90BetlC8Hx42juhaOJ9R1aiZ778N6i7cJhXw35VM6Cezs5uuf4UMQ%3D%3D&X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Date=20241031T073303Z&X-Amz-SignedHeaders=host&X-Amz-Expires=3599&X-Amz-Credential=ASIAXCQYYRKNOA4DTKYN%2F20241031%2Fus-east-1%2Fs3%2Faws4_request&X-Amz-Signature=5cf28d18d4b666d53f67455e5f54b1af794659c9be90d3e2492701c71a4a8d98\",\n            \"intouchPoints\": 0,\n            \"intouchSeriesId\": \"741888\",\n            \"redemptionType\": \"INTOUCH_REWARD\",\n            \"images\": [\n            \t\t{\n                \"name\": \"imageName\",\n                \"altText\": \"product picture\",\n                \"id\": null,\n                \"url\": \"mypic.com\",\n                \"isExternal\": true\n            }\n            ],\n            \"videos\": [\n            \t\t {\n                \"name\": \"videoName\",\n                \"altText\": \"product picture\",\n                \"id\": null,\n                \"url\": \"mypic.com\",\n                \"isExternal\": true\n            }\n            ]\n        },\n        \"pointsRedemption\": {\n            \"redeemedPoints\": 0,\n            \"pointsRedemptionRefIds\": [\n                null\n            ],\n            \"status\": {\n                \"success\": true,\n                \"code\": 200,\n                \"message\": \"Points redeemed successfully.\"\n            }\n        },\n        \"vouchers\": [],\n        \"vendorRewards\": {\n            \"rewards\": []\n        },\n        \"ownerId\": \"\"\n    }\n}"
                  },
                  "couponCodeCase=AS_IT_IS": {
                    "value": "{\n    \"status\": [\n        {\n            \"success\": true,\n            \"code\": 200,\n            \"message\": \"Reward issue transactions details fetched successfully\"\n        }\n    ],\n    \"data\": {\n        \"rewardTransactionId\": \"927488\",\n        \"customerId\": 345635195,\n        \"orgId\": 50339,\n        \"rewardIssueRefId\": \"1718950196\",\n        \"rewardTransactionDate\": 1747372097000,\n        \"status\": {\n            \"success\": true,\n            \"code\": 200,\n            \"message\": \"Reward issued successfully\"\n        },\n        \"quantity\": 1,\n        \"requestedQuantity\": 1,\n        \"groupRedemption\": false,\n        \"rewardDetails\": {\n            \"rewardId\": 139831,\n            \"languageCode\": \"en\",\n            \"name\": \"english rewardName\",\n            \"description\": \"hello\",\n            \"groups\": [],\n            \"imageId\": \"EXDR12987U\",\n            \"imageUrl\": \"https://s3.amazonaws.com/crm-staging-new-solutions/AutomationBrandMarvel/EXDR12987U?X-Amz-Security-Token=IQoJb3JpZ2luX2VjEJH%2F%2F%2F%2F%2F%2F%2F%2F%2F%2FwEaCXVzLWVhc3QtMSJHMEUCIE%2BeRWPDz0XYx7IjQEri%2F%2FpzAhgGJvwXWBc7mSOrRaokAiEAu1INaG1iTdyTeXZpI%2BZ%2BsNEI7VdoVdG1J53VmL4XhGAqvwIIWhAFGgw0ODY0NTY5ODYyNjYiDDcco1G%2BCL1zwPK2YCqcAvsLh13E3dAKtZs5Ru9O3VO8xXheuSsjubDb6csrxpnOT2cQsVnN3ma8Q1Pw7oG83pY717SdEVaRP951tkv9XwGgiB3sNCL1cs1AwLf2BDRvLvrb24PKzcJG3iQszRjFMHpujBuAiJt53AzY9Qy%2F6WlOXCRuuMVlUMeGdQNforRTTAkujk3kUtpsNzc%2B6gzDl04Gv2mlaznSAXweADW0cS6ajn79loWUlnaITZIwjJQUVxm4xgxFtki0%2BEhS%2FS1LnxHH3riQ%2Fixza0%2BvoHoQME8EbVXDEADWuS3ZeSVw50rQtult00p%2BIwjZt7IyM4RC0Xmh5lwKeZOcaKyhu762jyGRBgH8veMlXKTNNdfxvzqfV1ddB0x1hgbAIIYDMIz61cEGOp0BSEegrOVeFJSjbBOTD%2Fy2W1l0cN6PCvrap9NRmv3ws8pdbS89nSgPYdzF8hv6t%2F4fALargH1mUSahDGLUhSgmMH%2FcCIoXSb2iL4kJRycgr9j98hRQiJvdxh%2BHHIMimrvmDqpPzxOyZ0Ee7WDrSW9h1qsP5YD12cWAuU4hFvuM%2By077e1crCfa7EeNQHg2rKY04IvkxahZmvpcxVg9Dw%3D%3D&X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Date=20250527T085416Z&X-Amz-SignedHeaders=host&X-Amz-Expires=3600&X-Amz-Credential=ASIAXCQYYRKNHNOLXL4J%2F20250527%2Fus-east-1%2Fs3%2Faws4_request&X-Amz-Signature=c2a7ac120eddfac2cebd3ddd16dc2c8453af7598a923ee8b7591db92b931e107\",\n            \"intouchPoints\": 0,\n            \"intouchSeriesId\": \"96303\",\n            \"redemptionType\": \"INTOUCH_REWARD\",\n            \"images\": [],\n            \"videos\": []\n        },\n        \"pointsRedemption\": {\n            \"redeemedPoints\": 0,\n            \"vendorName\": \"test004\",\n            \"pointsRedemptionRefIds\": [\n                null\n            ],\n            \"status\": {\n                \"success\": true,\n                \"code\": 200,\n                \"message\": \"Points redeemed successfully.\"\n            },\n            \"vendorId\": 37\n        },\n        \"redemptionDetails\": {\n            \"redemptionType\": \"INTOUCH_REWARD\",\n            \"totalRedemptionValue\": null\n        },\n        \"vouchers\": [\n            {\n                \"code\": \"UPPERlower003\",\n                \"codeExpiry\": \"2025-06-15 00:00:00\",\n                \"codeExpiryDateTime\": \"2025-06-15T00:00:00Z\",\n                \"redemptionDetails\": {\n                    \"redemptionType\": \"INTOUCH_REWARD\",\n                    \"redemptionValue\": null\n                }\n            }\n        ],\n        \"vendorRewards\": {\n            \"rewards\": []\n        },\n        \"customFields\": {\n            \"CF2_eebced882025-05-27 13:57:18.344501\": \"defaultValue_2\",\n            \"CF1_50a6f4cf2025-05-27 13:57:18.344487\": \"defaultValue_1\"\n        },\n        \"richContentRO\": {},\n        \"ownerId\": \"\",\n        \"eventDateTime\": \"2025-05-16T05:08:17Z\",\n        \"issueDateTime\": \"2025-05-16T05:08:17Z\"\n    }\n}"
                  },
                  "couponCodeCase = LOWER": {
                    "value": "{\n    \"status\": [\n        {\n            \"success\": true,\n            \"code\": 200,\n            \"message\": \"Reward issue transactions details fetched successfully\"\n        }\n    ],\n    \"data\": {\n        \"rewardTransactionId\": \"927488\",\n        \"customerId\": 345635195,\n        \"orgId\": 50339,\n        \"rewardIssueRefId\": \"1718950196\",\n        \"rewardTransactionDate\": 1747372097000,\n        \"status\": {\n            \"success\": true,\n            \"code\": 200,\n            \"message\": \"Reward issued successfully\"\n        },\n        \"quantity\": 1,\n        \"requestedQuantity\": 1,\n        \"groupRedemption\": false,\n        \"rewardDetails\": {\n            \"rewardId\": 139831,\n            \"languageCode\": \"en\",\n            \"name\": \"english rewardName\",\n            \"description\": \"hello\",\n            \"groups\": [],\n            \"imageId\": \"EXDR12987U\",\n            \"imageUrl\": \"https://s3.amazonaws.com/crm-staging-new-solutions/AutomationBrandMarvel/EXDR12987U?X-Amz-Security-Token=IQoJb3JpZ2luX2VjED0aCXVzLWVhc3QtMSJGMEQCIBMtYIqqA45zbd98IEr9E71Kj1DEeRAJSqC93wjb8lGSAiB0LgFMxrBnv1r7RUh58yd18v%2BC83lrZ9g6zXR%2FNazSBCq%2FAggWEAUaDDQ4NjQ1Njk4NjI2NiIMRd70c%2FZFDj8w2SBHKpwC7Ou7tqNdHWY%2BuE%2FQZ5814yI%2BDOLJG6Ir%2BnmS6nwsy5g7ecoR2oNxZxRiqnrscSnpEleb%2FMint6ESMrYQshIjTQkNZGEorWYotRL6K5e3Hy8UlPAkaQ1Eki4pKgo%2FO0VPJtWz6Hpal1IG1Nutoqisd7MlTb%2B8DZ76BBEytZAoQ8BEDFcwn%2FgQgVxBonVEwWzjs%2FZpWAKq2YmT3KqZBoa6h0FrKKLkARhRch9y8xi6mjxXPefdGcGGfgqpdcukenb6tkPdE87S%2Bx0Xtf5T32QGfsKh%2B9sJmJGGTjqiek4D7JOr3e1vQ%2Fca1%2B1m23VL%2BcbxjMjb6F9wvYUTIdvMsJNi6ZGAXkWGhIQ3%2BhYmB%2FvxNTDBV6HehcSFpQ5Sw6Qwrtn7wQY6ngHalzhExI4S1K76x836fe9Vtq6PaxC2jHBAH2gPbqrErKmOGVZu%2FnQ80aKif8kT7zXNOaLWDezuL2gdDehMhwT%2BY%2FEovQRDroZtunb36yHT1cMNmMNFbSstSwuYuRnmyovdVtuQurRJVnKOHl%2FaTvx1SVV%2FNLNWveE0gbFACdO3uOBPpNmOTzWC2XBrnxvtlb3LKJ3jioAWZ5%2FI%2FadJhw%3D%3D&X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Date=20250603T123806Z&X-Amz-SignedHeaders=host&X-Amz-Expires=3599&X-Amz-Credential=ASIAXCQYYRKNOTBO6SUU%2F20250603%2Fus-east-1%2Fs3%2Faws4_request&X-Amz-Signature=a9aab95c85ef5cec0eba7d4c978c5754f0372e10f470d16dc29f7bb74a86952f\",\n            \"intouchPoints\": 0,\n            \"intouchSeriesId\": \"96303\",\n            \"redemptionType\": \"INTOUCH_REWARD\",\n            \"images\": [],\n            \"videos\": []\n        },\n        \"pointsRedemption\": {\n            \"redeemedPoints\": 0,\n            \"vendorName\": \"test004\",\n            \"pointsRedemptionRefIds\": [\n                null\n            ],\n            \"status\": {\n                \"success\": true,\n                \"code\": 200,\n                \"message\": \"Points redeemed successfully.\"\n            },\n            \"vendorId\": 37\n        },\n        \"redemptionDetails\": {\n            \"redemptionType\": \"INTOUCH_REWARD\",\n            \"totalRedemptionValue\": null\n        },\n        \"vouchers\": [\n            {\n                \"code\": \"upperlower003\",\n                \"codeExpiry\": \"2025-06-15 00:00:00\",\n                \"codeExpiryDateTime\": \"2025-06-15T00:00:00Z\",\n                \"redemptionDetails\": {\n                    \"redemptionType\": \"INTOUCH_REWARD\",\n                    \"redemptionValue\": null\n                }\n            }\n        ],\n        \"vendorRewards\": {\n            \"rewards\": []\n        },\n        \"customFields\": {\n            \"CF2_0122a5852025-06-03 13:59:47.736781\": \"defaultValue_2\",\n            \"CF1_c26dfde12025-06-03 13:59:47.736770\": \"defaultValue_1\"\n        },\n        \"richContentRO\": {},\n        \"ownerId\": \"\",\n        \"eventDateTime\": \"2025-05-16T05:08:17Z\",\n        \"issueDateTime\": \"2025-05-16T05:08:17Z\"\n    }\n}"
                  },
                  "couponCodeCase = UPPER": {
                    "value": "{\n    \"status\": [\n        {\n            \"success\": true,\n            \"code\": 200,\n            \"message\": \"Reward issue transactions details fetched successfully\"\n        }\n    ],\n    \"data\": {\n        \"rewardTransactionId\": \"927488\",\n        \"customerId\": 345635195,\n        \"orgId\": 50339,\n        \"rewardIssueRefId\": \"1718950196\",\n        \"rewardTransactionDate\": 1747372097000,\n        \"status\": {\n            \"success\": true,\n            \"code\": 200,\n            \"message\": \"Reward issued successfully\"\n        },\n        \"quantity\": 1,\n        \"requestedQuantity\": 1,\n        \"groupRedemption\": false,\n        \"rewardDetails\": {\n            \"rewardId\": 139831,\n            \"languageCode\": \"en\",\n            \"name\": \"english rewardName\",\n            \"description\": \"hello\",\n            \"groups\": [],\n            \"imageId\": \"EXDR12987U\",\n            \"imageUrl\": \"https://s3.amazonaws.com/crm-staging-new-solutions/AutomationBrandMarvel/EXDR12987U?X-Amz-Security-Token=IQoJb3JpZ2luX2VjED0aCXVzLWVhc3QtMSJHMEUCIDdiny0ZSOUcF7EibKTJv9zlt2jxzV6zRlC3HElGSPaUAiEAyVKkXW1Cn%2Bn1syBFpIZLBhd42sDxMmgjW6AtZNNHpBYqvwIIFhAFGgw0ODY0NTY5ODYyNjYiDPIQTS0SUoykHmQsbiqcAuzQPcwZ%2F94VKuZMpXFHPPoy5X15DHJOzOnJUUJ2ltixLHhf4fbHVdM033Gtr0Lm8obFVRRajrYnqzPcMEQIEQh%2BTWOQgE6vS5sCg3%2FtFDwFXWTtrL30pIcO%2Fps6DgBipaYWuJPP6sSEi3%2F9%2Bvvx99tjjnvTBkGjGsoJtnKs5PhqTVcgCjqm6742oqvEpsabdpmneuMtag87H5nkfrGuIJx6e9d97%2FJAw8PxQD%2BgC2IrhgoSwqSfMirrNKDtqHLgpMt7kQnMMfq2g9zo%2FFz%2FgRJJ8Q5ut7BQj7R2OpPFpQd15Kcwt45kEzztWqBJUKj2Yzgcrn6dSM1k5bRK9gfWCXJceXaTIWJPHYAz4ES6lkgpJZ1A8QMYsoyOWwu8MN3Z%2B8EGOp0BOJX4Zx2EF8rO3hiYXMxHlZws6g618MnVlUM9ds1yUrx1F4Ci2SngZSSsanX%2FO8GSTYEWCSDjAuX77RVIJC6giPZVJPz7UcfxpK6lB44QjLVynUdAwCJwB8pZgxGnwFFHgVUudF%2B5EC1SbmRnjP7pmB9Si50FpL8XsZnDoyAAzUv2qLQmYDQf9YLxhpZ1J0xy5onbcyB0OgZU4%2BFrcQ%3D%3D&X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Date=20250603T123853Z&X-Amz-SignedHeaders=host&X-Amz-Expires=3599&X-Amz-Credential=ASIAXCQYYRKNDPQM2XMS%2F20250603%2Fus-east-1%2Fs3%2Faws4_request&X-Amz-Signature=05c0598c0a4c5fcaf021d0cef464e0451ff4564400daa69e18c41dda0f9b5b3c\",\n            \"intouchPoints\": 0,\n            \"intouchSeriesId\": \"96303\",\n            \"redemptionType\": \"INTOUCH_REWARD\",\n            \"images\": [],\n            \"videos\": []\n        },\n        \"pointsRedemption\": {\n            \"redeemedPoints\": 0,\n            \"vendorName\": \"test004\",\n            \"pointsRedemptionRefIds\": [\n                null\n            ],\n            \"status\": {\n                \"success\": true,\n                \"code\": 200,\n                \"message\": \"Points redeemed successfully.\"\n            },\n            \"vendorId\": 37\n        },\n        \"redemptionDetails\": {\n            \"redemptionType\": \"INTOUCH_REWARD\",\n            \"totalRedemptionValue\": null\n        },\n        \"vouchers\": [\n            {\n                \"code\": \"UPPERLOWER003\",\n                \"codeExpiry\": \"2025-06-15 00:00:00\",\n                \"codeExpiryDateTime\": \"2025-06-15T00:00:00Z\",\n                \"redemptionDetails\": {\n                    \"redemptionType\": \"INTOUCH_REWARD\",\n                    \"redemptionValue\": null\n                }\n            }\n        ],\n        \"vendorRewards\": {\n            \"rewards\": []\n        },\n        \"customFields\": {\n            \"CF2_0122a5852025-06-03 13:59:47.736781\": \"defaultValue_2\",\n            \"CF1_c26dfde12025-06-03 13:59:47.736770\": \"defaultValue_1\"\n        },\n        \"richContentRO\": {},\n        \"ownerId\": \"\",\n        \"eventDateTime\": \"2025-05-16T05:08:17Z\",\n        \"issueDateTime\": \"2025-05-16T05:08:17Z\"\n    }\n}"
                  }
                },
                "schema": {
                  "oneOf": [
                    {
                      "title": "Sample response with DATA-SCOPE as OTHER",
                      "type": "object",
                      "properties": {
                        "status": {
                          "type": "array",
                          "items": {
                            "type": "object",
                            "properties": {
                              "success": {
                                "type": "boolean",
                                "example": true,
                                "default": true
                              },
                              "code": {
                                "type": "integer",
                                "example": 200,
                                "default": 0
                              },
                              "message": {
                                "type": "string",
                                "example": "Reward issue transactions details fetched successfully"
                              }
                            }
                          }
                        },
                        "data": {
                          "type": "object",
                          "properties": {
                            "rewardTransactionId": {
                              "type": "string",
                              "example": "32695"
                            },
                            "orgId": {
                              "type": "integer",
                              "example": 50672,
                              "default": 0
                            },
                            "customerId": {
                              "type": "integer",
                              "example": 123456,
                              "default": 0
                            },
                            "rewardIssueRefId": {
                              "type": "string",
                              "example": "transactionNumber"
                            },
                            "rewardTransactionDate": {
                              "type": "integer",
                              "example": 1653906525000,
                              "default": 0
                            },
                            "status": {
                              "type": "object",
                              "properties": {
                                "success": {
                                  "type": "boolean",
                                  "example": true,
                                  "default": true
                                },
                                "code": {
                                  "type": "integer",
                                  "example": 200,
                                  "default": 0
                                },
                                "message": {
                                  "type": "string",
                                  "example": "Reward issued successfully"
                                }
                              }
                            },
                            "quantity": {
                              "type": "integer",
                              "example": 1,
                              "default": 0
                            },
                            "requestedQuantity": {
                              "type": "integer",
                              "example": 0,
                              "default": 0
                            },
                            "groupRedemption": {
                              "type": "boolean",
                              "example": false,
                              "default": true
                            },
                            "rewardDetails": {
                              "type": "object",
                              "properties": {
                                "rewardId": {
                                  "type": "integer",
                                  "example": 663,
                                  "default": 0
                                },
                                "languageCode": {
                                  "type": "string",
                                  "example": "en"
                                },
                                "name": {
                                  "type": "string",
                                  "example": " INTOUCH Enabled"
                                },
                                "description": {
                                  "type": "string",
                                  "example": "Description"
                                },
                                "groups": {
                                  "type": "array"
                                },
                                "imageId": {
                                  "type": "string",
                                  "example": "EXDR12987U"
                                },
                                "imageUrl": {
                                  "type": "string",
                                  "example": "https://s3.amazonaws.com/crm-nightly-new-solutions/SAHANA_TEST/EXDR12987U?X-Amz-Security-Token=FwoGZXIvYXdzECEaDFH7mzvBmQUc2KqQryLWARZFNas29Qa%2BGT3bozjU69vumkm4K%2FACklgdn2iD2wQJKg3XS5P8SUKcwOriTGT6KXt2f8sq4VQGey7NBKPIfiwEC8YCkOdyt5iHPMt3Cdm1Op4ReXFKvJ2lPyOWEYS1lXYVYHhOHmKwFn%2BlTEIUgQBLna22wNZj4Pug8%2Fs8vBLw3Yf6vYvcrmoQpq0qpM9rPie3i4vMrDZgc03vB5mvlMNqXSwHtpW6X%2BNVX3%2FepQqCaPNj2gsy3%2BTZYK6FBnLyai7%2Fi5ndBqpp9B4Rpkc7%2Fj0T3D5xChAor%2BWMuQYyLRoAD76np90BetlC8Hx42juhaOJ9R1aiZ778N6i7cJhXw35VM6Cezs5uuf4UMQ%3D%3D&X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Date=20241031T073303Z&X-Amz-SignedHeaders=host&X-Amz-Expires=3599&X-Amz-Credential=ASIAXCQYYRKNOA4DTKYN%2F20241031%2Fus-east-1%2Fs3%2Faws4_request&X-Amz-Signature=5cf28d18d4b666d53f67455e5f54b1af794659c9be90d3e2492701c71a4a8d98"
                                },
                                "intouchPoints": {
                                  "type": "integer",
                                  "example": 0,
                                  "default": 0
                                },
                                "intouchSeriesId": {
                                  "type": "string",
                                  "example": "741888"
                                },
                                "redemptionType": {
                                  "type": "string",
                                  "example": "INTOUCH_REWARD"
                                },
                                "images": {
                                  "type": "array"
                                },
                                "videos": {
                                  "type": "array"
                                }
                              }
                            },
                            "pointsRedemption": {
                              "type": "object",
                              "properties": {
                                "redeemedPoints": {
                                  "type": "integer",
                                  "example": 0,
                                  "default": 0
                                },
                                "pointsRedemptionRefIds": {
                                  "type": "array",
                                  "items": {}
                                },
                                "status": {
                                  "type": "object",
                                  "properties": {
                                    "success": {
                                      "type": "boolean",
                                      "example": true,
                                      "default": true
                                    },
                                    "code": {
                                      "type": "integer",
                                      "example": 200,
                                      "default": 0
                                    },
                                    "message": {
                                      "type": "string",
                                      "example": "Points redeemed successfully."
                                    }
                                  }
                                }
                              }
                            },
                            "vouchers": {
                              "type": "array"
                            },
                            "vendorRewards": {
                              "type": "object",
                              "properties": {
                                "rewards": {
                                  "type": "array"
                                }
                              }
                            },
                            "ownerId": {
                              "type": "string",
                              "example": ""
                            }
                          }
                        }
                      }
                    },
                    {
                      "title": "With 'altText' for images and videos",
                      "type": "object",
                      "properties": {
                        "status": {
                          "type": "array",
                          "items": {
                            "type": "object",
                            "properties": {
                              "success": {
                                "type": "boolean",
                                "example": true,
                                "default": true
                              },
                              "code": {
                                "type": "integer",
                                "example": 200,
                                "default": 0
                              },
                              "message": {
                                "type": "string",
                                "example": "Reward issue transactions details fetched successfully"
                              }
                            }
                          }
                        },
                        "data": {
                          "type": "object",
                          "properties": {
                            "rewardTransactionId": {
                              "type": "string",
                              "example": "32695"
                            },
                            "orgId": {
                              "type": "integer",
                              "example": 50672,
                              "default": 0
                            },
                            "customerId": {
                              "type": "integer",
                              "example": 123456,
                              "default": 0
                            },
                            "rewardIssueRefId": {
                              "type": "string",
                              "example": "transactionNumber"
                            },
                            "rewardTransactionDate": {
                              "type": "integer",
                              "example": 1653906525000,
                              "default": 0
                            },
                            "status": {
                              "type": "object",
                              "properties": {
                                "success": {
                                  "type": "boolean",
                                  "example": true,
                                  "default": true
                                },
                                "code": {
                                  "type": "integer",
                                  "example": 200,
                                  "default": 0
                                },
                                "message": {
                                  "type": "string",
                                  "example": "Reward issued successfully"
                                }
                              }
                            },
                            "quantity": {
                              "type": "integer",
                              "example": 1,
                              "default": 0
                            },
                            "requestedQuantity": {
                              "type": "integer",
                              "example": 0,
                              "default": 0
                            },
                            "groupRedemption": {
                              "type": "boolean",
                              "example": false,
                              "default": true
                            },
                            "rewardDetails": {
                              "type": "object",
                              "properties": {
                                "rewardId": {
                                  "type": "integer",
                                  "example": 663,
                                  "default": 0
                                },
                                "languageCode": {
                                  "type": "string",
                                  "example": "en"
                                },
                                "name": {
                                  "type": "string",
                                  "example": " INTOUCH Enabled"
                                },
                                "description": {
                                  "type": "string",
                                  "example": "Description"
                                },
                                "groups": {
                                  "type": "array"
                                },
                                "imageId": {
                                  "type": "string",
                                  "example": "EXDR12987U"
                                },
                                "imageUrl": {
                                  "type": "string",
                                  "example": "https://s3.amazonaws.com/crm-nightly-new-solutions/SAHANA_TEST/EXDR12987U?X-Amz-Security-Token=FwoGZXIvYXdzECEaDFH7mzvBmQUc2KqQryLWARZFNas29Qa%2BGT3bozjU69vumkm4K%2FACklgdn2iD2wQJKg3XS5P8SUKcwOriTGT6KXt2f8sq4VQGey7NBKPIfiwEC8YCkOdyt5iHPMt3Cdm1Op4ReXFKvJ2lPyOWEYS1lXYVYHhOHmKwFn%2BlTEIUgQBLna22wNZj4Pug8%2Fs8vBLw3Yf6vYvcrmoQpq0qpM9rPie3i4vMrDZgc03vB5mvlMNqXSwHtpW6X%2BNVX3%2FepQqCaPNj2gsy3%2BTZYK6FBnLyai7%2Fi5ndBqpp9B4Rpkc7%2Fj0T3D5xChAor%2BWMuQYyLRoAD76np90BetlC8Hx42juhaOJ9R1aiZ778N6i7cJhXw35VM6Cezs5uuf4UMQ%3D%3D&X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Date=20241031T073303Z&X-Amz-SignedHeaders=host&X-Amz-Expires=3599&X-Amz-Credential=ASIAXCQYYRKNOA4DTKYN%2F20241031%2Fus-east-1%2Fs3%2Faws4_request&X-Amz-Signature=5cf28d18d4b666d53f67455e5f54b1af794659c9be90d3e2492701c71a4a8d98"
                                },
                                "intouchPoints": {
                                  "type": "integer",
                                  "example": 0,
                                  "default": 0
                                },
                                "intouchSeriesId": {
                                  "type": "string",
                                  "example": "741888"
                                },
                                "redemptionType": {
                                  "type": "string",
                                  "example": "INTOUCH_REWARD"
                                },
                                "images": {
                                  "type": "array",
                                  "items": {
                                    "type": "object",
                                    "properties": {
                                      "name": {
                                        "type": "string",
                                        "example": "imageName"
                                      },
                                      "altText": {
                                        "type": "string",
                                        "example": "product picture"
                                      },
                                      "id": {},
                                      "url": {
                                        "type": "string",
                                        "example": "mypic.com"
                                      },
                                      "isExternal": {
                                        "type": "boolean",
                                        "example": true,
                                        "default": true
                                      }
                                    }
                                  }
                                },
                                "videos": {
                                  "type": "array",
                                  "items": {
                                    "type": "object",
                                    "properties": {
                                      "name": {
                                        "type": "string",
                                        "example": "videoName"
                                      },
                                      "altText": {
                                        "type": "string",
                                        "example": "product picture"
                                      },
                                      "id": {},
                                      "url": {
                                        "type": "string",
                                        "example": "mypic.com"
                                      },
                                      "isExternal": {
                                        "type": "boolean",
                                        "example": true,
                                        "default": true
                                      }
                                    }
                                  }
                                }
                              }
                            },
                            "pointsRedemption": {
                              "type": "object",
                              "properties": {
                                "redeemedPoints": {
                                  "type": "integer",
                                  "example": 0,
                                  "default": 0
                                },
                                "pointsRedemptionRefIds": {
                                  "type": "array",
                                  "items": {}
                                },
                                "status": {
                                  "type": "object",
                                  "properties": {
                                    "success": {
                                      "type": "boolean",
                                      "example": true,
                                      "default": true
                                    },
                                    "code": {
                                      "type": "integer",
                                      "example": 200,
                                      "default": 0
                                    },
                                    "message": {
                                      "type": "string",
                                      "example": "Points redeemed successfully."
                                    }
                                  }
                                }
                              }
                            },
                            "vouchers": {
                              "type": "array"
                            },
                            "vendorRewards": {
                              "type": "object",
                              "properties": {
                                "rewards": {
                                  "type": "array"
                                }
                              }
                            },
                            "ownerId": {
                              "type": "string",
                              "example": ""
                            }
                          }
                        }
                      }
                    },
                    {
                      "title": "couponCodeCase=AS_IT_IS",
                      "type": "object",
                      "properties": {
                        "status": {
                          "type": "array",
                          "items": {
                            "type": "object",
                            "properties": {
                              "success": {
                                "type": "boolean",
                                "example": true,
                                "default": true
                              },
                              "code": {
                                "type": "integer",
                                "example": 200,
                                "default": 0
                              },
                              "message": {
                                "type": "string",
                                "example": "Reward issue transactions details fetched successfully"
                              }
                            }
                          }
                        },
                        "data": {
                          "type": "object",
                          "properties": {
                            "rewardTransactionId": {
                              "type": "string",
                              "example": "927488"
                            },
                            "customerId": {
                              "type": "integer",
                              "example": 345635195,
                              "default": 0
                            },
                            "orgId": {
                              "type": "integer",
                              "example": 50339,
                              "default": 0
                            },
                            "rewardIssueRefId": {
                              "type": "string",
                              "example": "1718950196"
                            },
                            "rewardTransactionDate": {
                              "type": "integer",
                              "example": 1747372097000,
                              "default": 0
                            },
                            "status": {
                              "type": "object",
                              "properties": {
                                "success": {
                                  "type": "boolean",
                                  "example": true,
                                  "default": true
                                },
                                "code": {
                                  "type": "integer",
                                  "example": 200,
                                  "default": 0
                                },
                                "message": {
                                  "type": "string",
                                  "example": "Reward issued successfully"
                                }
                              }
                            },
                            "quantity": {
                              "type": "integer",
                              "example": 1,
                              "default": 0
                            },
                            "requestedQuantity": {
                              "type": "integer",
                              "example": 1,
                              "default": 0
                            },
                            "groupRedemption": {
                              "type": "boolean",
                              "example": false,
                              "default": true
                            },
                            "rewardDetails": {
                              "type": "object",
                              "properties": {
                                "rewardId": {
                                  "type": "integer",
                                  "example": 139831,
                                  "default": 0
                                },
                                "languageCode": {
                                  "type": "string",
                                  "example": "en"
                                },
                                "name": {
                                  "type": "string",
                                  "example": "english rewardName"
                                },
                                "description": {
                                  "type": "string",
                                  "example": "hello"
                                },
                                "groups": {
                                  "type": "array"
                                },
                                "imageId": {
                                  "type": "string",
                                  "example": "EXDR12987U"
                                },
                                "imageUrl": {
                                  "type": "string",
                                  "example": "https://s3.amazonaws.com/crm-staging-new-solutions/AutomationBrandMarvel/EXDR12987U?X-Amz-Security-Token=IQoJb3JpZ2luX2VjEJH%2F%2F%2F%2F%2F%2F%2F%2F%2F%2FwEaCXVzLWVhc3QtMSJHMEUCIE%2BeRWPDz0XYx7IjQEri%2F%2FpzAhgGJvwXWBc7mSOrRaokAiEAu1INaG1iTdyTeXZpI%2BZ%2BsNEI7VdoVdG1J53VmL4XhGAqvwIIWhAFGgw0ODY0NTY5ODYyNjYiDDcco1G%2BCL1zwPK2YCqcAvsLh13E3dAKtZs5Ru9O3VO8xXheuSsjubDb6csrxpnOT2cQsVnN3ma8Q1Pw7oG83pY717SdEVaRP951tkv9XwGgiB3sNCL1cs1AwLf2BDRvLvrb24PKzcJG3iQszRjFMHpujBuAiJt53AzY9Qy%2F6WlOXCRuuMVlUMeGdQNforRTTAkujk3kUtpsNzc%2B6gzDl04Gv2mlaznSAXweADW0cS6ajn79loWUlnaITZIwjJQUVxm4xgxFtki0%2BEhS%2FS1LnxHH3riQ%2Fixza0%2BvoHoQME8EbVXDEADWuS3ZeSVw50rQtult00p%2BIwjZt7IyM4RC0Xmh5lwKeZOcaKyhu762jyGRBgH8veMlXKTNNdfxvzqfV1ddB0x1hgbAIIYDMIz61cEGOp0BSEegrOVeFJSjbBOTD%2Fy2W1l0cN6PCvrap9NRmv3ws8pdbS89nSgPYdzF8hv6t%2F4fALargH1mUSahDGLUhSgmMH%2FcCIoXSb2iL4kJRycgr9j98hRQiJvdxh%2BHHIMimrvmDqpPzxOyZ0Ee7WDrSW9h1qsP5YD12cWAuU4hFvuM%2By077e1crCfa7EeNQHg2rKY04IvkxahZmvpcxVg9Dw%3D%3D&X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Date=20250527T085416Z&X-Amz-SignedHeaders=host&X-Amz-Expires=3600&X-Amz-Credential=ASIAXCQYYRKNHNOLXL4J%2F20250527%2Fus-east-1%2Fs3%2Faws4_request&X-Amz-Signature=c2a7ac120eddfac2cebd3ddd16dc2c8453af7598a923ee8b7591db92b931e107"
                                },
                                "intouchPoints": {
                                  "type": "integer",
                                  "example": 0,
                                  "default": 0
                                },
                                "intouchSeriesId": {
                                  "type": "string",
                                  "example": "96303"
                                },
                                "redemptionType": {
                                  "type": "string",
                                  "example": "INTOUCH_REWARD"
                                },
                                "images": {
                                  "type": "array"
                                },
                                "videos": {
                                  "type": "array"
                                }
                              }
                            },
                            "pointsRedemption": {
                              "type": "object",
                              "properties": {
                                "redeemedPoints": {
                                  "type": "integer",
                                  "example": 0,
                                  "default": 0
                                },
                                "vendorName": {
                                  "type": "string",
                                  "example": "test004"
                                },
                                "pointsRedemptionRefIds": {
                                  "type": "array",
                                  "items": {}
                                },
                                "status": {
                                  "type": "object",
                                  "properties": {
                                    "success": {
                                      "type": "boolean",
                                      "example": true,
                                      "default": true
                                    },
                                    "code": {
                                      "type": "integer",
                                      "example": 200,
                                      "default": 0
                                    },
                                    "message": {
                                      "type": "string",
                                      "example": "Points redeemed successfully."
                                    }
                                  }
                                },
                                "vendorId": {
                                  "type": "integer",
                                  "example": 37,
                                  "default": 0
                                }
                              }
                            },
                            "redemptionDetails": {
                              "type": "object",
                              "properties": {
                                "redemptionType": {
                                  "type": "string",
                                  "example": "INTOUCH_REWARD"
                                },
                                "totalRedemptionValue": {}
                              }
                            },
                            "vouchers": {
                              "type": "array",
                              "items": {
                                "type": "object",
                                "properties": {
                                  "code": {
                                    "type": "string",
                                    "example": "UPPERlower003"
                                  },
                                  "codeExpiry": {
                                    "type": "string",
                                    "example": "2025-06-15 00:00:00"
                                  },
                                  "codeExpiryDateTime": {
                                    "type": "string",
                                    "example": "2025-06-15T00:00:00Z"
                                  },
                                  "redemptionDetails": {
                                    "type": "object",
                                    "properties": {
                                      "redemptionType": {
                                        "type": "string",
                                        "example": "INTOUCH_REWARD"
                                      },
                                      "redemptionValue": {}
                                    }
                                  }
                                }
                              }
                            },
                            "vendorRewards": {
                              "type": "object",
                              "properties": {
                                "rewards": {
                                  "type": "array"
                                }
                              }
                            },
                            "customFields": {
                              "type": "object",
                              "properties": {
                                "CF2_eebced882025-05-27 13:57:18.344501": {
                                  "type": "string",
                                  "example": "defaultValue_2"
                                },
                                "CF1_50a6f4cf2025-05-27 13:57:18.344487": {
                                  "type": "string",
                                  "example": "defaultValue_1"
                                }
                              }
                            },
                            "richContentRO": {
                              "type": "object",
                              "properties": {}
                            },
                            "ownerId": {
                              "type": "string",
                              "example": ""
                            },
                            "eventDateTime": {
                              "type": "string",
                              "example": "2025-05-16T05:08:17Z"
                            },
                            "issueDateTime": {
                              "type": "string",
                              "example": "2025-05-16T05:08:17Z"
                            }
                          }
                        }
                      }
                    },
                    {
                      "title": "couponCodeCase = LOWER",
                      "type": "object",
                      "properties": {
                        "status": {
                          "type": "array",
                          "items": {
                            "type": "object",
                            "properties": {
                              "success": {
                                "type": "boolean",
                                "example": true,
                                "default": true
                              },
                              "code": {
                                "type": "integer",
                                "example": 200,
                                "default": 0
                              },
                              "message": {
                                "type": "string",
                                "example": "Reward issue transactions details fetched successfully"
                              }
                            }
                          }
                        },
                        "data": {
                          "type": "object",
                          "properties": {
                            "rewardTransactionId": {
                              "type": "string",
                              "example": "927488"
                            },
                            "customerId": {
                              "type": "integer",
                              "example": 345635195,
                              "default": 0
                            },
                            "orgId": {
                              "type": "integer",
                              "example": 50339,
                              "default": 0
                            },
                            "rewardIssueRefId": {
                              "type": "string",
                              "example": "1718950196"
                            },
                            "rewardTransactionDate": {
                              "type": "integer",
                              "example": 1747372097000,
                              "default": 0
                            },
                            "status": {
                              "type": "object",
                              "properties": {
                                "success": {
                                  "type": "boolean",
                                  "example": true,
                                  "default": true
                                },
                                "code": {
                                  "type": "integer",
                                  "example": 200,
                                  "default": 0
                                },
                                "message": {
                                  "type": "string",
                                  "example": "Reward issued successfully"
                                }
                              }
                            },
                            "quantity": {
                              "type": "integer",
                              "example": 1,
                              "default": 0
                            },
                            "requestedQuantity": {
                              "type": "integer",
                              "example": 1,
                              "default": 0
                            },
                            "groupRedemption": {
                              "type": "boolean",
                              "example": false,
                              "default": true
                            },
                            "rewardDetails": {
                              "type": "object",
                              "properties": {
                                "rewardId": {
                                  "type": "integer",
                                  "example": 139831,
                                  "default": 0
                                },
                                "languageCode": {
                                  "type": "string",
                                  "example": "en"
                                },
                                "name": {
                                  "type": "string",
                                  "example": "english rewardName"
                                },
                                "description": {
                                  "type": "string",
                                  "example": "hello"
                                },
                                "groups": {
                                  "type": "array"
                                },
                                "imageId": {
                                  "type": "string",
                                  "example": "EXDR12987U"
                                },
                                "imageUrl": {
                                  "type": "string",
                                  "example": "https://s3.amazonaws.com/crm-staging-new-solutions/AutomationBrandMarvel/EXDR12987U?X-Amz-Security-Token=IQoJb3JpZ2luX2VjED0aCXVzLWVhc3QtMSJGMEQCIBMtYIqqA45zbd98IEr9E71Kj1DEeRAJSqC93wjb8lGSAiB0LgFMxrBnv1r7RUh58yd18v%2BC83lrZ9g6zXR%2FNazSBCq%2FAggWEAUaDDQ4NjQ1Njk4NjI2NiIMRd70c%2FZFDj8w2SBHKpwC7Ou7tqNdHWY%2BuE%2FQZ5814yI%2BDOLJG6Ir%2BnmS6nwsy5g7ecoR2oNxZxRiqnrscSnpEleb%2FMint6ESMrYQshIjTQkNZGEorWYotRL6K5e3Hy8UlPAkaQ1Eki4pKgo%2FO0VPJtWz6Hpal1IG1Nutoqisd7MlTb%2B8DZ76BBEytZAoQ8BEDFcwn%2FgQgVxBonVEwWzjs%2FZpWAKq2YmT3KqZBoa6h0FrKKLkARhRch9y8xi6mjxXPefdGcGGfgqpdcukenb6tkPdE87S%2Bx0Xtf5T32QGfsKh%2B9sJmJGGTjqiek4D7JOr3e1vQ%2Fca1%2B1m23VL%2BcbxjMjb6F9wvYUTIdvMsJNi6ZGAXkWGhIQ3%2BhYmB%2FvxNTDBV6HehcSFpQ5Sw6Qwrtn7wQY6ngHalzhExI4S1K76x836fe9Vtq6PaxC2jHBAH2gPbqrErKmOGVZu%2FnQ80aKif8kT7zXNOaLWDezuL2gdDehMhwT%2BY%2FEovQRDroZtunb36yHT1cMNmMNFbSstSwuYuRnmyovdVtuQurRJVnKOHl%2FaTvx1SVV%2FNLNWveE0gbFACdO3uOBPpNmOTzWC2XBrnxvtlb3LKJ3jioAWZ5%2FI%2FadJhw%3D%3D&X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Date=20250603T123806Z&X-Amz-SignedHeaders=host&X-Amz-Expires=3599&X-Amz-Credential=ASIAXCQYYRKNOTBO6SUU%2F20250603%2Fus-east-1%2Fs3%2Faws4_request&X-Amz-Signature=a9aab95c85ef5cec0eba7d4c978c5754f0372e10f470d16dc29f7bb74a86952f"
                                },
                                "intouchPoints": {
                                  "type": "integer",
                                  "example": 0,
                                  "default": 0
                                },
                                "intouchSeriesId": {
                                  "type": "string",
                                  "example": "96303"
                                },
                                "redemptionType": {
                                  "type": "string",
                                  "example": "INTOUCH_REWARD"
                                },
                                "images": {
                                  "type": "array"
                                },
                                "videos": {
                                  "type": "array"
                                }
                              }
                            },
                            "pointsRedemption": {
                              "type": "object",
                              "properties": {
                                "redeemedPoints": {
                                  "type": "integer",
                                  "example": 0,
                                  "default": 0
                                },
                                "vendorName": {
                                  "type": "string",
                                  "example": "test004"
                                },
                                "pointsRedemptionRefIds": {
                                  "type": "array",
                                  "items": {}
                                },
                                "status": {
                                  "type": "object",
                                  "properties": {
                                    "success": {
                                      "type": "boolean",
                                      "example": true,
                                      "default": true
                                    },
                                    "code": {
                                      "type": "integer",
                                      "example": 200,
                                      "default": 0
                                    },
                                    "message": {
                                      "type": "string",
                                      "example": "Points redeemed successfully."
                                    }
                                  }
                                },
                                "vendorId": {
                                  "type": "integer",
                                  "example": 37,
                                  "default": 0
                                }
                              }
                            },
                            "redemptionDetails": {
                              "type": "object",
                              "properties": {
                                "redemptionType": {
                                  "type": "string",
                                  "example": "INTOUCH_REWARD"
                                },
                                "totalRedemptionValue": {}
                              }
                            },
                            "vouchers": {
                              "type": "array",
                              "items": {
                                "type": "object",
                                "properties": {
                                  "code": {
                                    "type": "string",
                                    "example": "upperlower003"
                                  },
                                  "codeExpiry": {
                                    "type": "string",
                                    "example": "2025-06-15 00:00:00"
                                  },
                                  "codeExpiryDateTime": {
                                    "type": "string",
                                    "example": "2025-06-15T00:00:00Z"
                                  },
                                  "redemptionDetails": {
                                    "type": "object",
                                    "properties": {
                                      "redemptionType": {
                                        "type": "string",
                                        "example": "INTOUCH_REWARD"
                                      },
                                      "redemptionValue": {}
                                    }
                                  }
                                }
                              }
                            },
                            "vendorRewards": {
                              "type": "object",
                              "properties": {
                                "rewards": {
                                  "type": "array"
                                }
                              }
                            },
                            "customFields": {
                              "type": "object",
                              "properties": {
                                "CF2_0122a5852025-06-03 13:59:47.736781": {
                                  "type": "string",
                                  "example": "defaultValue_2"
                                },
                                "CF1_c26dfde12025-06-03 13:59:47.736770": {
                                  "type": "string",
                                  "example": "defaultValue_1"
                                }
                              }
                            },
                            "richContentRO": {
                              "type": "object",
                              "properties": {}
                            },
                            "ownerId": {
                              "type": "string",
                              "example": ""
                            },
                            "eventDateTime": {
                              "type": "string",
                              "example": "2025-05-16T05:08:17Z"
                            },
                            "issueDateTime": {
                              "type": "string",
                              "example": "2025-05-16T05:08:17Z"
                            }
                          }
                        }
                      }
                    },
                    {
                      "title": "couponCodeCase = UPPER",
                      "type": "object",
                      "properties": {
                        "status": {
                          "type": "array",
                          "items": {
                            "type": "object",
                            "properties": {
                              "success": {
                                "type": "boolean",
                                "example": true,
                                "default": true
                              },
                              "code": {
                                "type": "integer",
                                "example": 200,
                                "default": 0
                              },
                              "message": {
                                "type": "string",
                                "example": "Reward issue transactions details fetched successfully"
                              }
                            }
                          }
                        },
                        "data": {
                          "type": "object",
                          "properties": {
                            "rewardTransactionId": {
                              "type": "string",
                              "example": "927488"
                            },
                            "customerId": {
                              "type": "integer",
                              "example": 345635195,
                              "default": 0
                            },
                            "orgId": {
                              "type": "integer",
                              "example": 50339,
                              "default": 0
                            },
                            "rewardIssueRefId": {
                              "type": "string",
                              "example": "1718950196"
                            },
                            "rewardTransactionDate": {
                              "type": "integer",
                              "example": 1747372097000,
                              "default": 0
                            },
                            "status": {
                              "type": "object",
                              "properties": {
                                "success": {
                                  "type": "boolean",
                                  "example": true,
                                  "default": true
                                },
                                "code": {
                                  "type": "integer",
                                  "example": 200,
                                  "default": 0
                                },
                                "message": {
                                  "type": "string",
                                  "example": "Reward issued successfully"
                                }
                              }
                            },
                            "quantity": {
                              "type": "integer",
                              "example": 1,
                              "default": 0
                            },
                            "requestedQuantity": {
                              "type": "integer",
                              "example": 1,
                              "default": 0
                            },
                            "groupRedemption": {
                              "type": "boolean",
                              "example": false,
                              "default": true
                            },
                            "rewardDetails": {
                              "type": "object",
                              "properties": {
                                "rewardId": {
                                  "type": "integer",
                                  "example": 139831,
                                  "default": 0
                                },
                                "languageCode": {
                                  "type": "string",
                                  "example": "en"
                                },
                                "name": {
                                  "type": "string",
                                  "example": "english rewardName"
                                },
                                "description": {
                                  "type": "string",
                                  "example": "hello"
                                },
                                "groups": {
                                  "type": "array"
                                },
                                "imageId": {
                                  "type": "string",
                                  "example": "EXDR12987U"
                                },
                                "imageUrl": {
                                  "type": "string",
                                  "example": "https://s3.amazonaws.com/crm-staging-new-solutions/AutomationBrandMarvel/EXDR12987U?X-Amz-Security-Token=IQoJb3JpZ2luX2VjED0aCXVzLWVhc3QtMSJHMEUCIDdiny0ZSOUcF7EibKTJv9zlt2jxzV6zRlC3HElGSPaUAiEAyVKkXW1Cn%2Bn1syBFpIZLBhd42sDxMmgjW6AtZNNHpBYqvwIIFhAFGgw0ODY0NTY5ODYyNjYiDPIQTS0SUoykHmQsbiqcAuzQPcwZ%2F94VKuZMpXFHPPoy5X15DHJOzOnJUUJ2ltixLHhf4fbHVdM033Gtr0Lm8obFVRRajrYnqzPcMEQIEQh%2BTWOQgE6vS5sCg3%2FtFDwFXWTtrL30pIcO%2Fps6DgBipaYWuJPP6sSEi3%2F9%2Bvvx99tjjnvTBkGjGsoJtnKs5PhqTVcgCjqm6742oqvEpsabdpmneuMtag87H5nkfrGuIJx6e9d97%2FJAw8PxQD%2BgC2IrhgoSwqSfMirrNKDtqHLgpMt7kQnMMfq2g9zo%2FFz%2FgRJJ8Q5ut7BQj7R2OpPFpQd15Kcwt45kEzztWqBJUKj2Yzgcrn6dSM1k5bRK9gfWCXJceXaTIWJPHYAz4ES6lkgpJZ1A8QMYsoyOWwu8MN3Z%2B8EGOp0BOJX4Zx2EF8rO3hiYXMxHlZws6g618MnVlUM9ds1yUrx1F4Ci2SngZSSsanX%2FO8GSTYEWCSDjAuX77RVIJC6giPZVJPz7UcfxpK6lB44QjLVynUdAwCJwB8pZgxGnwFFHgVUudF%2B5EC1SbmRnjP7pmB9Si50FpL8XsZnDoyAAzUv2qLQmYDQf9YLxhpZ1J0xy5onbcyB0OgZU4%2BFrcQ%3D%3D&X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Date=20250603T123853Z&X-Amz-SignedHeaders=host&X-Amz-Expires=3599&X-Amz-Credential=ASIAXCQYYRKNDPQM2XMS%2F20250603%2Fus-east-1%2Fs3%2Faws4_request&X-Amz-Signature=05c0598c0a4c5fcaf021d0cef464e0451ff4564400daa69e18c41dda0f9b5b3c"
                                },
                                "intouchPoints": {
                                  "type": "integer",
                                  "example": 0,
                                  "default": 0
                                },
                                "intouchSeriesId": {
                                  "type": "string",
                                  "example": "96303"
                                },
                                "redemptionType": {
                                  "type": "string",
                                  "example": "INTOUCH_REWARD"
                                },
                                "images": {
                                  "type": "array"
                                },
                                "videos": {
                                  "type": "array"
                                }
                              }
                            },
                            "pointsRedemption": {
                              "type": "object",
                              "properties": {
                                "redeemedPoints": {
                                  "type": "integer",
                                  "example": 0,
                                  "default": 0
                                },
                                "vendorName": {
                                  "type": "string",
                                  "example": "test004"
                                },
                                "pointsRedemptionRefIds": {
                                  "type": "array",
                                  "items": {}
                                },
                                "status": {
                                  "type": "object",
                                  "properties": {
                                    "success": {
                                      "type": "boolean",
                                      "example": true,
                                      "default": true
                                    },
                                    "code": {
                                      "type": "integer",
                                      "example": 200,
                                      "default": 0
                                    },
                                    "message": {
                                      "type": "string",
                                      "example": "Points redeemed successfully."
                                    }
                                  }
                                },
                                "vendorId": {
                                  "type": "integer",
                                  "example": 37,
                                  "default": 0
                                }
                              }
                            },
                            "redemptionDetails": {
                              "type": "object",
                              "properties": {
                                "redemptionType": {
                                  "type": "string",
                                  "example": "INTOUCH_REWARD"
                                },
                                "totalRedemptionValue": {}
                              }
                            },
                            "vouchers": {
                              "type": "array",
                              "items": {
                                "type": "object",
                                "properties": {
                                  "code": {
                                    "type": "string",
                                    "example": "UPPERLOWER003"
                                  },
                                  "codeExpiry": {
                                    "type": "string",
                                    "example": "2025-06-15 00:00:00"
                                  },
                                  "codeExpiryDateTime": {
                                    "type": "string",
                                    "example": "2025-06-15T00:00:00Z"
                                  },
                                  "redemptionDetails": {
                                    "type": "object",
                                    "properties": {
                                      "redemptionType": {
                                        "type": "string",
                                        "example": "INTOUCH_REWARD"
                                      },
                                      "redemptionValue": {}
                                    }
                                  }
                                }
                              }
                            },
                            "vendorRewards": {
                              "type": "object",
                              "properties": {
                                "rewards": {
                                  "type": "array"
                                }
                              }
                            },
                            "customFields": {
                              "type": "object",
                              "properties": {
                                "CF2_0122a5852025-06-03 13:59:47.736781": {
                                  "type": "string",
                                  "example": "defaultValue_2"
                                },
                                "CF1_c26dfde12025-06-03 13:59:47.736770": {
                                  "type": "string",
                                  "example": "defaultValue_1"
                                }
                              }
                            },
                            "richContentRO": {
                              "type": "object",
                              "properties": {}
                            },
                            "ownerId": {
                              "type": "string",
                              "example": ""
                            },
                            "eventDateTime": {
                              "type": "string",
                              "example": "2025-05-16T05:08:17Z"
                            },
                            "issueDateTime": {
                              "type": "string",
                              "example": "2025-05-16T05:08:17Z"
                            }
                          }
                        }
                      }
                    }
                  ]
                }
              }
            }
          },
          "400": {
            "description": "400",
            "content": {
              "application/json": {
                "examples": {
                  "Invalid rewardTransactionId": {
                    "value": "{\n    \"status\": [\n        {\n            \"success\": false,\n            \"code\": 10007,\n            \"message\": \"Transaction not found\"\n        }\n    ],\n    \"data\": null\n}"
                  }
                },
                "schema": {
                  "type": "object",
                  "properties": {
                    "status": {
                      "type": "array",
                      "items": {
                        "type": "object",
                        "properties": {
                          "success": {
                            "type": "boolean",
                            "example": false,
                            "default": true
                          },
                          "code": {
                            "type": "integer",
                            "example": 10007,
                            "default": 0
                          },
                          "message": {
                            "type": "string",
                            "example": "Transaction not found"
                          }
                        }
                      }
                    },
                    "data": {}
                  }
                }
              }
            }
          }
        },
        "deprecated": false,
        "x-readme": {
          "code-samples": [
            {
              "language": "curl",
              "code": "/curl --location 'http://crm-nightly-new.cc.capillarytech.com/api_gateway/rewards/core/v1.1/reward-transactions/<rewardTransactionId>' \\\ne--header 'DATA-SCOPE: OTHER' \\\nx--header 'DATA-SCOPE-ORG: 50672' \\\n--header 'Authorization: Basic a2FyXzE6MzE1MmQyMzRiNzA='",
              "name": "Sample request with DATA-SCOPE as OTHER"
            },
            {
              "language": "curl",
              "code": "curl --location 'https://crm-staging-new.cc.capillarytech.com/api_gateway/rewards/core/v1.1/reward-transactions/927488?couponCodeCase=AS_IT_IS' \\\n--header 'Accept: application/json' \\\n--header 'Content-Type: application/json' \\\n--header 'DATA-SCOPE: ALL' \\\n--header 'Authorization: Basic dGlsbGF1dG9fMTIzOjI3NDFlNjJiZTZiZTYyNGY2N2IwMGQxZDZhNWQ2MWJm'",
              "name": "couponCodeCase=AS_IT_IS"
            },
            {
              "language": "curl",
              "code": "curl --location 'https://crm-staging-new.cc.capillarytech.com/api_gateway/rewards/core/v1.1/reward-transactions/927488?couponCodeCase=UPPER' \\\n--header 'Accept: application/json' \\\n--header 'Content-Type: application/json' \\\n--header 'DATA-SCOPE: ALL' \\\n--header 'Authorization: Basic dGlsbGF1dG9fMTIzOjI3NDFlNjJiZTZiZTYyNGY2N2IwMGQxZDZhNWQ2MWJm'",
              "name": "couponCodeCase = UPPER"
            },
            {
              "language": "curl",
              "code": "curl --location 'https://crm-staging-new.cc.capillarytech.com/api_gateway/rewards/core/v1.1/reward-transactions/927488?couponCodeCase=LOWER' \\\n--header 'Accept: application/json' \\\n--header 'Content-Type: application/json' \\\n--header 'DATA-SCOPE: ALL' \\\n--header 'Authorization: Basic dGlsbGF1dG9fMTIzOjI3NDFlNjJiZTZiZTYyNGY2N2IwMGQxZDZhNWQ2MWJm'",
              "name": "couponCodeCase = LOWER"
            }
          ],
          "samples-languages": [
            "curl"
          ]
        }
      }
    }
  },
  "x-readme": {
    "headers": [
      {
        "key": "Content-Type",
        "value": "application/json"
      },
      {
        "key": "Accept",
        "value": "application/json"
      }
    ],
    "explorer-enabled": true,
    "proxy-enabled": true
  },
  "x-readme-fauxas": true
}
```
